...
## 零件总长度 (Width) 提取规则（非常重要！）
注意：如果图纸中标注了孔阵列的跨度（例如 "8x25(=200)"），这通常**不是**零件的总长度！
总长度 = 左边距 + 阵列跨度 + 右边距。
例如：左边距是15，阵列跨度是200，右边距是对称的15，那么总长度 (width) 应该是 15 + 200 + 15 = 230。
请仔细查看图纸边缘的总体尺寸标注，或者通过边距推算总长度。不要盲目将孔阵列的跨度作为零件总长度。
